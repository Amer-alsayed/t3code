...
function sortThreadsByNewest(left: Thread, right: Thread): number {
...
function T3Wordmark() {
  return (
    <span className="shrink-0 text-[18px] font-bold leading-none tracking-tighter text-foreground">
      T3
    </span>
  );
}
...
export default function Sidebar() {
  ...
  const projectThreadsByProjectId = useMemo(() => {
    const groupedThreads = new Map<ProjectId, Thread[]>();
    for (const thread of threads) {
      const existing = groupedThreads.get(thread.projectId);
      if (existing) {
    ...
    for (const [projectId, projectThreads] of groupedThreads) {
      groupedThreads.set(projectId, projectThreads.toSorted(sortThreadsByNewest));
    }
    return groupedThreads;
  }, [threads]);
  const latestThreadByProjectId = useMemo(() => {
    const map = new Map<ProjectId, Thread>();
    for (const [projectId, projectThreads] of projectThreadsByProjectId) {
      const latestThread = projectThreads[0];
      if (latestThread) {
        map.set(projectId, latestThread);
      }
    }
    return map;
  ...
  const projectSections = useMemo(
    () =>
      projects.map((project) => ({
        project,
        threads: projectThreadsByProjectId.get(project.id) ?? [],
      })),
    [projectThreadsByProjectId, projects],
  );
    ...
    (
      ...
      if (storedDraftThread) {
        return (async () => {
          ...
          navigateToThread(storedDraftThread.threadId);
        })();
      }
  ...
  const focusMostRecentThreadForProject = useCallback(
    ...
    [latestThreadByProjectId, navigateToThread],
  );
  ...
  const handleProjectContextMenu = useCallback(
    async (projectId: ProjectId, position: { x: number; y: number }) => {
      ...
      const confirmed = await api.dialogs.confirm(
        [
          ...
          projectThreads.length > 0
            ? `This will also delete ${projectThreads.length} thread${
      ...
      try {
        const routeThread = routeThreadId ? threads.find((thread) => thread.id === routeThreadId) : null;
        const routeDraftThread = routeThreadId ? getDraftThread(routeThreadId) : null;
        const shouldNavigateAway =
          routeThread?.projectId === projectId || routeDraftThread?.projectId === projectId;
        const fallbackThreadId =
          threads.find((thread) => thread.projectId !== projectId)?.id ?? null;
  ...
  return (
    <>
      {isElectron ? (
        <>
          <SidebarHeader className={`drag-region h-[36px] flex-row items-center gap-2 py-0 ${typeof navigator !== "undefined" && navigator.userAgent.includes("Mac") ? "pl-[82px] pr-4" : "px-4"}`}>
            {wordmark}
              ...
              return (
                ...
                >
                  <SidebarMenuItem>
                    ...
                    <CollapsibleContent>
                      <SidebarMenuSub className="sidebar-thread-list mx-1 my-0 w-full translate-x-0 gap-0 px-1.5 py-0">
        ...
        {addingProject ? (
          <>
            ...
            {isElectron && (
              <button
                type="button"
                className="mb-2 flex w-full items-center justify-center rounded-md border border-border px-2 py-1.5 text-xs text-muted-foreground transition-[color,background-color,border-color,transform] duration-150 hover:bg-secondary active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <div className="flex gap-2">
              <button
                type="button"
                className="flex-1 rounded-md bg-primary px-2 py-1 text-xs font-medium text-primary-foreground transition-[color,background-color,transform] duration-150 hover:bg-primary/90 active:scale-[0.97]"
              ...
              <button
                type="button"
                className="flex-1 rounded-md border border-border px-2 py-1 text-xs text-muted-foreground/80 transition-[color,background-color,border-color,transform] duration-150 hover:bg-secondary active:scale-[0.97]"
          ...
          <button
            type="button"
            className="flex w-full items-center justify-center gap-1 rounded-md border border-dashed border-border py-2 text-xs text-muted-foreground/70 transition-[color,background-color,border-color,transform] duration-150 hover:border-ring hover:text-muted-foreground active:scale-[0.97]"
